...
import galleryShelf from "@/assets/gallery-shelf.jpg";
  ...
  head: () => ({
    meta: [
      { title: "Elora Homes — Premium Short Stays in Bengaluru" },
      {
        name: "description",
        content:
          "Thoughtfully designed short stays in Bengaluru. Spaces styled for every story.",
      },
...
const storyImages = [galleryLounge, galleryBed, galleryKitchen, galleryShelf, galleryTable, galleryBedTwo];
  ...
  return (
    <>
      ...
      <section className="relative flex min-h-[100svh] items-end overflow-hidden bg-espresso">
        <img
          src={heroImage}
          alt="Elora Homes interior in Bengaluru"
          width={1920}
          height={1280}
          className="absolute inset-0 h-full w-full object-cover motion-safe:animate-[heroZoom_18s_ease-out_forwards]"
        />
        ...
        <div className="relative mx-auto flex min-h-[100svh] w-full max-w-7xl flex-col justify-end px-6 pb-16 md:px-10 md:pb-24 lg:px-16">
          ...
          <h1 className="mt-7 max-w-4xl text-[clamp(3.2rem,8vw,7.5rem)] leading-[0.9] text-ivory">
            Spaces Styled
            <br />
            for Every Story.
          </h1>
          <div className="mt-10 flex items-end justify-between gap-8">
            ...
            <span className="hidden pb-1 text-[0.65rem] tracking-[0.25em] text-ivory/60 uppercase md:block">
              Scroll to explore ↓
            </span>
          </div>
          <div className="mt-9">
      ...
      {/* Featured stays — immersive rather than a conventional card grid */}
      <section className="bg-ivory px-6 py-24 md:px-10 md:py-32 lg:px-16">
        <div className="mx-auto max-w-7xl">
          <Reveal>
            <SectionHeading eyebrow="Featured Stays" title="Two homes." intro="Each with its own rhythm, character and way of making you want to stay a little longer." />
          </Reveal>
            ...
            {[{ stay: stays[0]!, image: stayOne, number: "01" }, { stay: stays[1]!, image: stayTwo, number: "02" }].map(({ stay, image, number }, i) => (
              <article key={stay.slug} className="group relative min-h-[78svh] overflow-hidden bg-espresso">
                <img
                  ...
                  className="absolute inset-0 h-full w-full object-cover transition-transform duration-[1800ms] ease-out group-hover:scale-[1.035]"
                />
                ...
                <div className="relative flex min-h-[78svh] flex-col justify-between p-7 md:p-12 lg:p-16">
                  <div className="flex justify-between text-[0.65rem] tracking-[0.25em] text-ivory/65 uppercase">
                    <span>{number}</span>
                  ...
                  <div className="max-w-2xl">
                    ...
                    <div className="mt-8 flex flex-wrap items-center gap-x-6 gap-y-3 text-[0.68rem] tracking-[0.15em] text-ivory/65 uppercase">
                      <span>{stay.guests} guests</span>
                      <span>{stay.bedrooms} bedrooms</span>
                      <span>{stay.bathrooms} baths</span>
                    </div>
                    <div className="mt-9">
                      <ActionLink to="/stays/$slug" params={{ slug: stay.slug }} variant="gold">
                        Discover the home
                      </ActionLink>
                    </div>
                  </div>
      ...
      <section className="bg-beige">
        <div className="mx-auto grid max-w-7xl gap-0 px-6 md:grid-cols-[0.95fr_1.05fr] md:px-10 lg:px-16">
          <div className="relative order-2 md:order-1">
            <div className="sticky top-0 flex min-h-[70svh] items-center py-12 md:h-screen md:py-20">
          ...
          <div className="order-1 py-24 md:order-2 md:py-40 md:pl-16 lg:pl-24">
            <Reveal>
              <p className="eyebrow text-gold">The Elora feeling</p>
              <h2 className="mt-6 max-w-xl text-[clamp(2.8rem,5vw,5rem)] leading-[0.95] text-forest">
                Notice the light. Stay for the details.
              </h2>
            </Reveal>
            <div className="mt-20 space-y-28 md:mt-32">
              {[
                ["01", "Light", "Rooms shaped around the way Bengaluru light moves through a home."],
                ["02", "Texture", "Linen, wood, warm finishes and small details that make a space feel lived in."],
                ["03", "Ease", "Everything considered so the stay itself can feel effortless."],
              ].map(([number, title, copy], i) => (
                <Reveal key={number} delay={i * 80}>
                  <span className="text-xs tracking-[0.22em] text-gold uppercase">{number}</span>
                  <h3 className="mt-5 text-4xl text-forest md:text-5xl">{title}</h3>
                  <p className="mt-5 max-w-md text-sm leading-[1.9] font-light text-muted-foreground">{copy}</p>
                </Reveal>
              ))}
            </div>
          </div>
          ...
          <Reveal>
            <div className="mb-14 flex items-end justify-between gap-8">
              <div>
                ...
                <h2 className="mt-5 max-w-2xl text-[clamp(2.8rem,5vw,5.2rem)] leading-[0.95] text-ivory">A collection of quiet moments.</h2>
              </div>
              <span className="hidden text-[0.65rem] tracking-[0.22em] text-ivory/50 uppercase md:block">Scroll through</span>
            </div>
          </Reveal>
          <div className="grid gap-3 md:grid-cols-12 md:gap-5">
            {storyImages.map((src, i) => (
              <Reveal key={i} delay={i * 70} className={`overflow-hidden ${i === 0 || i === 5 ? "md:col-span-7" : "md:col-span-5"}`}>
                <img
                  src={src}
                  alt="Elora Homes interior"
                  width={1400}
                  height={1100}
                  loading="lazy"
                  className={`w-full object-cover transition-transform duration-[1600ms] ease-out hover:scale-[1.025] ${i === 0 || i === 5 ? "aspect-[16/10]" : "aspect-[4/5]"}`}
                />
              </Reveal>
            ))}
          </div>
      ...
      <section className="bg-ivory px-6 py-24 md:px-10 md:py-36 lg:px-16">
        <div className="mx-auto max-w-7xl">
          <Reveal>
            <SectionHeading eyebrow="Instagram" title="@elorahomesinn" intro="Follow the homes, the details and the moments between stays." align="center" />
          </Reveal>
          <div className="mt-16 grid grid-cols-2 gap-3 md:grid-cols-3 md:gap-5">
            {[galleryKitchen, galleryLounge, galleryBedTwo].map((src, i) => (
              <Reveal key={i} delay={i * 100} className="overflow-hidden">
                <a href={brand.instagramUrl} target="_blank" rel="noreferrer" aria-label="Open Elora Homes on Instagram">
                  <img
                    src={src}
                    ...
                    height={1200}
                    loading="lazy"
                    className="aspect-square w-full object-cover transition-transform duration-[1200ms] ease-out hover:scale-[1.04]"
                  />
                </a>
              </Reveal>
            ))}
          ...
          <Reveal className="mt-12 text-center">
            <ActionLink href={brand.instagramUrl} variant="outline">View Instagram</ActionLink>
          </Reveal>
        </div>
      </section>
      ...
      <section className="relative min-h-[75svh] overflow-hidden bg-espresso">
        <img src={stayTwo} alt="The Golden Hour at Elora Homes" width={1600} height={1200} loading="lazy" className="absolute inset-0 h-full w-full object-cover" />
        ...
        <div className="relative mx-auto flex min-h-[75svh] max-w-7xl items-end px-6 py-20 md:px-10 md:py-28 lg:px-16">
          <Reveal>
            <p className="eyebrow text-gold-pale">Elora Homes</p>
            <h2 className="mt-6 max-w-3xl text-[clamp(3rem,7vw,7rem)] leading-[0.9] text-ivory">Spaces Styled for Every Story.</h2>
            <div className="mt-10">
